...
export default function VideoHero({ 
  ...
  useEffect(() => {
    ...
    // Función para ajustar el tamaño del video para cubrir completamente
    const adjustVideoSize = () => {
      ...
      // Si el video tiene dimensiones válidas
      if (video.videoWidth > 0 && video.videoHeight > 0) {
        let scale = 1;
        
        if (videoAspectRatio > containerAspectRatio) {
          // Video es más ancho - escalar por altura
          scale = containerHeight / video.videoHeight;
        } else {
          // Video es más alto - escalar por ancho
          scale = containerWidth / video.videoWidth;
        }
        
        // Asegurar que cubra completamente (agregar un poco más para evitar bordes)
        ...
        // Fallback: usar object-fit cover
    ...
    const handleScroll = () => {
      ...
      if (!isDesktop) {
        ...
        const currentTransform = video.style.transform.includes('translate(-50%, -50%)') 
          ? 'translate(-50%, -50%)' 
          : 'translate(-50%, -50%)';
      ...
      // Mantener el centrado y agregar parallax
      video.style.transform = `translate(-50%, calc(-50% + ${scrolled * 0.25}px))`;
    };
        ...
        <div className="absolute top-4 left-4 z-50 bg-black/80 text-white p-4 rounded text-xs font-mono max-w-md">
          ...
          {videoRef.current && (
            <>
              ...
              <div>VideoWidth: {videoRef.current.videoWidth}</div>
              <div>VideoHeight: {videoRef.current.videoHeight}</div>
            </>
          )}
        </div>
      )}
