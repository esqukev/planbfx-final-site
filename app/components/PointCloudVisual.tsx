...
export default function PointCloudVisual({ className = '' }: PointCloudVisualProps) {
  ...
  return (
    <div
      ref={mountRef}
      className={`relative mx-auto flex items-center justify-center w-full max-w-[min(50vw,182px)] aspect-square flex-shrink-0 sm:max-w-[min(65vw,260px)] md:max-w-[min(85vw,400px)] lg:max-w-[min(85vw,440px)] ${className}`}
      style={{ minHeight: 'clamp(180px, 50vw, 280px)' }}
      aria-hidden="true"
    />
  );
}
